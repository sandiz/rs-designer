...
import PixiCanvasEntry from './drawer.pixicanvasentry';
 ...
 * on the zoom level.
 */
export default class PixiCanvas extends Drawer {
    /**
    ...
    constructor(container, params) {
         ...
         * @type {function}
         */
        this.EntryClass = PixiCanvasEntry;
         ...
         * @type {boolean}
         */
        this.supportsIntersectionObserver = false;//'IntersectionObserver' in window;
    }
    ...
    updateSize() {
        ...
        this.maxCanvasElementWidth = totalWidth;
        ...
        this.canvases.forEach((entry, i) => {
            // reset entry
            this.updateDimensions(entry, this.width, this.height);

            entry.clearWave();
        });
    }

    getGraphicsHandle(type) {
        const entry = this.canvases[0]
        if (type == "timeline") {
            return [entry.timelineGr, entry.timeline];
        }
    }
    ...
    addCanvas() {
        ...
        entry.usesIntersectionObserver = false;//this.supportsIntersectionObserver;
     ...
     *
     * @private
     * @param {PixiCanvasEntry} entry Target entry
    ...
    updateDimensions(entry, width, height) {
        ...
        entry.updateDimensions(elementWidth, totalWidth, width, height, this.wrapper.clientWidth);
    ...
    clearWave() {
        this.canvases.forEach(entry => {
            ...
            if (this.canvases.length === 1) {
                entry.usesIntersectionObserver = false;
            }
        });
    }
     ...
     * @return {PixiCanvasEntry} The canvas entry associated with `element`.
     */
