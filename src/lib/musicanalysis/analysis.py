...
def resample(oldMatrix, newWidth):
    print("resampling...")
...
def draw(dB, width, height, file):
    print("exporting..." + str(width) + " " + str(height))
    pixels = dB
    heightFactor = 2.2
...
def export_to_png(dB, width, height, file):
    ...
    newdB = resample(newdB, width)
    draw(newdB, width, height, file)
...
def cqt_librosa(y, sr):
    ...
    print("width: " + str(newWidth))

    mode = 'dump'
    # if mode == 'dump':
    path = os.path.join(sys.argv[2], "cqt.png")
    #np.save(path, dB)
